...
export const Route = createFileRoute("/careers")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Join Just Bite. Open roles in production, marketing and business development.",
      },
    ],
  }),
  component: Careers,
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Build a <em className="not-italic text-gradient-gold">plastic-free world</em>
          </>
        }
        sub="Bring your craft to a young, mission-led team. We're hiring across operations, marketing and R&D."
      />
        ...
        <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-2 gap-12">
          <div>
            ...
            <div className="mt-8 space-y-4">
              {ROLES.map((r) => (
                <div
                  key={r.title}
                  className="glass rounded-2xl p-6 flex items-center justify-between gap-4 hover:border-primary/60 transition"
                >
                  <div>
          ...
          <form onSubmit={submit} className="glass rounded-2xl p-8 space-y-4">
            ...
            <textarea
              ...
              className="w-full rounded-lg bg-input/50 border border-border px-4 py-3 text-sm focus:outline-none focus:border-primary"
            />
            <button className="w-full rounded-full bg-gradient-gold py-3 text-sm font-semibold text-onyx shadow-gold hover:scale-[1.02] transition">
              Submit application
            </button>
          </form>
        </div>
